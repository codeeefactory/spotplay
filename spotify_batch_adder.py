...
def collect_hourly_tracks(
    ...
    if stateless_rotation:
        start_index = datetime.now(timezone.utc).toordinal() % len(style_queries)
    else:
    ...
    if persist_state and not stateless_rotation:
...
def main() -> int:
    ...
    parser.add_argument("--stateless-rotation", action="store_true", help="Rotate query start by UTC date instead of saving state. Useful on serverless hosts.")
        ...
        if args.hourly_update:
            if args.generate_only:
                ...
                tracks = collect_hourly_tracks(
                    ...
                    persist_state=False,
                    stateless_rotation=args.stateless_rotation,
                )
                save_outputs({"hourly_update": tracks}, output_dir)
                return 0
